...
const Hero = () => {
  const imageArray = [assets.cal1, assets.cal, assets.cal2, assets.cal4];
  ...
  // Move to the next image every 5 seconds unless paused
  useEffect(() => {
    if (!isPaused) {
  ...
  // Trigger slide-up animation when scrolling down
  useEffect(() => {
    const handleScroll = () => {
    ...
    >
      ...
      <div className="w-full sm:w-1/2 flex flex-col items-start justify-center p-4 min-h-[600px] bg-white bg-opacity-0">
        <div className="text-[#414141] text-left">
          ...
          >
            Based in Ottawa, we're happy to bring your creative nail art ideas
            to life. Join us for a day of inspiration and artistry.
          </p>
        </div>
      </div>
